...
def mlpc_2(train_data):
	cls = Classifier(classname="weka.classifiers.functions.MultilayerPerceptron", options=["-N", "2"])
	cls.build_classifier(train_data)
	return cls
	#pass
#
...
def evaluate_results(results_dir_str, model_scores_file_list, classes_list):
	...
	summary_header = 'Algorithm,ROC_Area,PRC_Area,F1_Score,Accuracy' + '\n'
	...
	for model_scores_file in model_scores_file_list:
		...
		try:
			...
			print "Exception while Evaluating area under ROC and PR curves: " + model_scores_file.split('/')[-1][:-4] + str(e)
			cur_roc_auc = 1
			cur_prc_auc = 1
		#

		cur_results = [cur_algorithm, cur_roc_auc, cur_prc_auc, cur_f1, cur_accuracy]
